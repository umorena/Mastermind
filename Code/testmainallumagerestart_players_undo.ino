...
void setup() {
  ...
  int l=0;
  int ct=0;
  for (int k=0; ct<NUM_LEDS;k++){ //pour allumer toutes les leds au démarrage
    if (k==4){
      l+=1;
      k=0;
    }
...
void loop() {
  ...
  if (sol[0]==9){ //initialement, forcément à 9 
    ...
    if (digitalRead(undo)==LOW){
      if (j!=0){
        j-=1;
        strip.setPixelColor(led[i][j],0,0,0);
        strip.show();
    ...
    if (j==4){ //si on est à la fin de la ligne
      //***** comparaison solution et réponse 
      //***** allumer leds qui analysent
      ...
      //****** si c'est juste, on rentre dans une boucle d'attente de pression du bouton restart
      //***** ouverture trappe ! 
