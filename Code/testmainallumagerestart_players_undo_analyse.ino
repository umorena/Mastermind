...
int led[12][4]={{0,1,2,3},{7,6,5,4},{8,9,10,11},{15,14,13,12},{16,17,18,19},{23,22,21,20},{24,25,26,27},{31,30,29,28},{32,33,34,35},{39,38,37,36},{40,41,42,43},{47,46,45,44}};//liste de toutes les leds
int ledAnaly[12][4]={{0,1,2,3},{7,6,5,4},{8,9,10,11},{15,14,13,12},{16,17,18,19},{23,22,21,20},{24,25,26,27},{31,30,29,28},{32,33,34,35},{39,38,37,36},{40,41,42,43},{47,46,45,44}};
...
void setup() {
  ...
  for (int k=0; cpt<NUM_LEDS;k++){ //pour allumer toutes les leds au démarrage
    ...
    strip.setPixelColor(ledAnaly[l][k],100,100,100);
    cpt+=1;
  }  
  cpt=0;
  for (cpt;cpt<4;cpt++){ 
  ...
  /* JEU PRINCIPAL */
    ...
    if (j==4){ //si on est à la fin de la ligne
      ...
      for (int vertVar=0;vertVar<vert;vertVar++){
        stripAnaly.setPixelColor(led[i][jA],0,255,0);
        jA++;
      }
      for (int rougeVar=0;rougeVar<rouge;rougeVar++){
        stripAnaly.setPixelColor(led[i][jA],255,0,0);
        jA++;
      }
      stripAnaly.show();
